...
<script>
	$(function() {
		findTaskListByProjectId();
	});
	function findTaskListByProjectId() {
		 
		 var table =$('#tableGroupList').DataTable(
		{
			"ordering":false,
			"searching":false,
			"ajax" : "manage/findTaskListByProjectId?projectId=" + <%=request.getAttribute("projectId")%> ,
			"columns" : [
					{
						"data" : "id"
					},
	...
	function showAddGroupPanel() {
		$("#btnUpdateGroup").hide();
		$("#btnAddGroup").show();
		$("#groupModalTitle").text("Task添加");
		$('#groupModal').modal('show');
	}
	function addGroup() {
		$.ajax({
			type : 'POST',
			url : "manage/insertTask",
			data : {
				"task.name" : $("#txtGroupName").val(),
				"task.projectId" : $("#txtProjectId").val(),
				"task.username" : $("#secParentGroup").val(),
				"task.time" : $("#txtGroupCode").val()
			},
	...
	function showUpdateGroupPanel(obj){
		...
		$.ajax({
			type : 'POST',
			url : "manage/findTaskById?taskId="+groupId,
			async : false,
			success : function(data) {
				$("#btnUpdateGroup").show();
				$("#btnAddGroup").hide();
				...
				$("#secParentGroup").val(data["username"]);
				$("#txtGroupCode").val(data["time"]);
				$('#groupModal').modal('show');
			}
		});
	...
	function updateGroup(){
		$.ajax({
			type : 'POST',
			url : "manage/updateTask",
			data : {
				"task.name" : $("#txtGroupName").val(),
				"task.id" : $("#txtGroupId").val(),
				"task.username" : $("#secParentGroup").val(),
				"task.time" : $("#txtGroupCode").val()
	...
	function deleteGroup(obj) {
		if(confirm("确定要删除")){
			$.ajax({
				type : 'POST',
				url : "manage/deleteTask?taskId=" + $(obj).parent("td").siblings().eq(0).text(),
				async : false,
				success : function(data) {
					if (data > 0) {
						alert("删除成功");
			...
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						...
						<h4 class="modal-title" id="groupModalTitle">Task编辑</h4>
					</div>
